...
public:
    ...
    bool test4()
    {
        ...
        for (int i = 0; i < capacity; i++)
        {
            ASSERT_TRUE(list.insert_back(i*20))
        }
        ASSERT_FALSE(list.insert_front(100))
        ASSERT_TRUE(list.remove(3))

        list.remove_back();
        list.remove_front();
        ASSERT_TRUE(list.insert_front(222))
        ASSERT_TRUE(list.insert(999, 2))
        ASSERT_TRUE(list.insert(888, 9))
        int num = list.size();
        for (int i = 0; i < num; i++)
        {
            list.remove_front();
...
public:
    ...
    bool test5()
    {
        ...
        ASSERT_FALSE(list.insert_back(100));
